...
class ChartViewCell: UITableViewCell, BaseViewCell {
    ...
    private let lineColor: UIColor = UIColor(hexString: "#BEDB86")
    ...
    @IBAction func doSeemore() {
        guard let action = seemoreAction else { return }
        action()
    }
...
extension ChartViewCell {
    ...
    func configure(_ cals: [Double], type: ChartType,  value: String, action: (()->Void)? = nil) {
        self.seemoreAction = action
        seemoreButton.isHidden = action == nil
        ...
        let lineModel = ChartLineModel(chartPoints: chartPoints, lineColor: lineColor, lineWidth: 1, animDuration: 0.7, animDelay: 0)
        ...
        let circleView = ChartPointEllipseView(center: chartPointModel.screenLoc, diameter: 12)
            ...
            circleView.fillColor = self.lineColor
